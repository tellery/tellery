...
export abstract class Block extends Entity {
  ...
  toModel(workspaceId: string): BlockEntity {
    ...
    entity.resources = this.resources
  ...
  return manager.query(
    `WITH RECURSIVE result AS (
        SELECT id, children, resources, "storyId" FROM blocks WHERE id = $${
          params.length + 1
        } UNION SELECT origin.id, origin.children, origin.resources, origin."storyId" FROM result JOIN blocks origin
        ON origin."storyId" = result."storyId" AND (origin.id = ANY(result.children) OR origin.id = ANY(result.resources))
      )
      ${updateSql}`,
    [...params, rootBlockId],
  )
